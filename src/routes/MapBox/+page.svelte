		...
		$alerts.data.forEach((phenomena) => {
			phenomena.Alerts.forEach((alert) => {
				...
				const geometry: number[][] =
					typeof current.geometry === 'string' ? JSON.parse(current.geometry) : current.geometry;
						...
						<div class="bg-neutral-800 border-b border-neutral-700">
								...
								<div>
									<button
										...
										on:click={() => {
											map.easeTo({
												center: getCoords(
													typeof alert.AlertHistory[0].geometry === 'string'
														? JSON.parse(alert.AlertHistory[0].geometry)
														: alert.AlertHistory[0].geometry
												),
												zoom: 9
											});
							...
							<div class="flex flex-wrap justify-start space-x-2 px-2">
								{#each alert.AlertHistory[0].AlertUGC as county}
									<div
										class="bg-blue-700 font-semibold text-sm whitespace-nowrap px-1 mb-1 rounded"
									>
										{county.CountyFIPS.name}, {county.CountyFIPS
											.StateFIPS_CountyFIPS_stateToStateFIPS.abbreviation}
									</div>
								{/each}
							</div>
						</div>
